...
class AudioService {
  ...
  Future<Map<String, dynamic>> stopLoopRecording() =>
      _useWeb ? _webBridge.stopLoopRecording() : _bridge.stopLoopRecording();
  ...
  Future<void> clearLoop() =>
      _useWeb ? _webBridge.clearLoop() : _bridge.clearLoop();
  ...
  List<double> getLayerWaveform(int index, int numSamples) =>
      _useWeb ? _webBridge.getLayerWaveform(index, numSamples) : List.filled(numSamples, 0.0);
  ...
  Future<void> playDrumHit(String track) =>
      _useWeb ? _webBridge.playDrumHit(track) : _bridge.safeInvoke('playSound', {'key': track});
  ...
  Stream<double> get loopPosition => _useWeb ? _webBridge.loopPosition : const Stream.empty();
  Stream<double> get inputLevel => _useWeb ? _webBridge.inputLevel : const Stream.empty();
  Stream<Map<String, dynamic>> get overdubAutoStop => _useWeb ? _webBridge.overdubAutoStop : const Stream.empty();
  ...
  Future<Map<String, dynamic>> songLabLoadTrack(Uint8List audioData, String name, String stemType) =>
      _useWeb ? _webBridge.songLabLoadTrack(audioData, name, stemType) : Future.value({'error': 'native_not_supported'});
  Future<void> songLabPlay() =>
      _useWeb ? _webBridge.songLabPlay() : Future.value();
  Future<void> songLabPause() =>
      _useWeb ? _webBridge.songLabPause() : Future.value();
  Future<void> songLabStop() =>
      _useWeb ? _webBridge.songLabStop() : Future.value();
  Future<void> songLabSeek(double position) =>
      _useWeb ? _webBridge.songLabSeek(position) : Future.value();
  Future<void> songLabSetTrackVolume(int index, double volume) =>
      _useWeb ? _webBridge.songLabSetTrackVolume(index, volume) : Future.value();
  ...
  Future<void> songLabSetSpeed(double speed) =>
      _useWeb ? _webBridge.songLabSetSpeed(speed) : Future.value();
  ...
  List<double> songLabGetWaveform(int trackIndex, int numSamples) =>
      _useWeb ? _webBridge.songLabGetWaveform(trackIndex, numSamples) : List.filled(numSamples, 0.0);
  Future<void> songLabClearAll() =>
      _useWeb ? _webBridge.songLabClearAll() : Future.value();
  ...
  Stream<double> get songLabPosition =>
      _useWeb ? _webBridge.songLabPosition : const Stream.empty();
