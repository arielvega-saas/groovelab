...
class _SongLabTabState extends ConsumerState<SongLabTab>
  ...
  Future<void> _importAudio() async {
    ...
    try {
      ...
      setState(() {
        ...
        _isPlaying = false;
        _isImporting = true;
        _importStatusText = 'Cargando archivo...';
      });

      // Small delay to let the overlay render before heavy processing
      await Future.delayed(const Duration(milliseconds: 50));

      if (mounted) setState(() => _importStatusText = 'Procesando audio...');

      final info = await audio.songLabLoadTrack(bytes, fileName, 'fullMix');

      if (mounted) setState(() => _importStatusText = 'Generando forma de onda...');
      ...
      // Run pitch detection
      if (mounted) setState(() => _importStatusText = 'Analizando audio...');
      try {
      ...
      if (mounted) {
        setState(() => _importStatusText = 'Listo');
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(
            'Error al importar: ${e.toString().length > 80 ? '${e.toString().substring(0, 80)}...' : e}',
            style: AppFonts.outfit(color: Colors.white, fontSize: 12),
          ),
  ...
  Future<void> _doExport() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Export failed: $e', style: AppFonts.outfit(color: Colors.white)),
  ...
  Future<void> _saveModifiedMp3() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Saved & exported successfully', style: AppFonts.outfit(color: Colors.white)),
              ...
              SongLabHeader(
                ...
                onSave: () {
                  ...
                  if (project != null) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Project saved', style: AppFonts.outfit(color: Colors.white)),
                        backgroundColor: AppColors.bgPanel,
                        behavior: SnackBarBehavior.floating,
                ...
                onDiscard: () {
                  showDialog(
                    ...
                    builder: (ctx) => AlertDialog(
                      ...
                      title: Text('Discard Project?', style: AppFonts.outfit(
                      ...
                      actions: [
                        ...
                        TextButton(
                          ...
                          child: Text('Discard', style: AppFonts.outfit(
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Transform.scale(
                        scale: 0.95 + _separatingAnim.value * 0.1,
                        child: Container(
                          width: 72,
                          height: 72,
                          decoration: BoxDecoration(
                            ...
                            color: AppColors.bgPanel,
                            border: Border.all(
                              color: ModuleColors.songLab.withValues(alpha: 0.25 + _separatingAnim.value * 0.20),
                              width: 2,
                            ),
                            boxShadow: [
                              ...AppColors.neumorphicRaised(scale: 0.6),
                              BoxShadow(
                                color: ModuleColors.songLab.withValues(alpha: 0.10 + _separatingAnim.value * 0.12),
                                blurRadius: 20,
                                spreadRadius: -2,
                              ),
                            ],
                          ),
                          child: _importStatusText == 'Listo'
                              ? Icon(Icons.check_rounded, size: 32, color: AppColors.accent2)
                              : Icon(
                                  Icons.music_note_rounded,
                                  size: 30,
                                  color: ModuleColors.songLab.withValues(alpha: 0.6 + _separatingAnim.value * 0.4),
                                ),
                        ),
                      ),
                      const SizedBox(height: 20),
                      // Progress bar
                      if (_importStatusText != 'Listo')
                        SizedBox(
                          width: 180,
                          child: ClipRRect(
                            ...
                            child: LinearProgressIndicator(
                              ...
                              valueColor: AlwaysStoppedAnimation<Color>(ModuleColors.songLab),
                              minHeight: 4,
                            ),
                          ),
                        ),
                      ...
                      Text(
                        ...
                        style: AppFonts.outfit(
                          fontSize: 13,
                          fontWeight: FontWeight.w600,
                          color: _importStatusText == 'Listo'
                              ? AppColors.accent2
                              : AppColors.textSecondary,
                        ),
                      ),
                      if (_trackName != null && _importStatusText != 'Listo') ...[
                        const SizedBox(height: 4),
                        Text(
                          _trackName!,
                          style: AppFonts.outfit(
              ...
              if (_detectedNotes.isNotEmpty) ...[
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text(
                        ...
                        style: AppFonts.jetBrainsMono(fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.accent),
                      ),
                    ],
                  ),
                ),
                ...
                if (_getCurrentChord().isNotEmpty) ...[
                  ...
                  Container(
                    ...
                    child: Text(
                      _getCurrentChord(),
                      style: AppFonts.jetBrainsMono(fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.accent2),
                    ),
                  ),
                ],
              ],
              ...
              Column(
                children: [
                  ...
                  Text(
                    ...
                    style: AppFonts.outfit(fontSize: 11, color: AppColors.warm),
                  ),
                ],
              )
            else if (_duration > 0)
  ...
  Widget _buildStemRow(Stem stem) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: isSolo
            ? [BoxShadow(color: color.withValues(alpha: 0.12), blurRadius: 6)]
            : null,
      ),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          // ── Colored track lane with mini waveform ──
          Container(
            height: 28,
              ...
              child: Row(
                children: [
                  ...
                  Container(
                    width: 3,
                    decoration: BoxDecoration(
                      color: isMuted ? AppColors.textMuted.withValues(alpha: 0.3) : color,
                  ...
                  Expanded(
                    child: RepaintBoundary(
                      child: CustomPaint(
                        painter: _StemLaneWaveformPainter(
          ...
          // ── Main row: LED dots + name + volume slider + controls ──
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 3),
            child: Row(
              children: [
                ...
                // Stem icon
                Icon(Stem.iconForType(stem.type), size: 12,
          ...
          Padding(
            padding: const EdgeInsets.only(left: 32, top: 0, bottom: 2),
            child: Row(
              children: [
                Text(
                  'PAN',
                ...
                const SizedBox(width: 4),
                Text('L', style: AppFonts.jetBrainsMono(fontSize: 7, color: AppColors.textMuted)),
                Expanded(
                  child: SliderTheme(
                ...
                Text('R', style: AppFonts.jetBrainsMono(fontSize: 7, color: AppColors.textMuted)),
                const SizedBox(width: 2),
                SizedBox(
                  width: 24,
                  child: Text(
  ...
  /// LED-style status dot indicator.
  Widget _stemLedDot({required Color color, required bool isActive}) {
    return Container(
      width: 7,
      height: 7,
      decoration: BoxDecoration(
  ...
  /// LED-style solo/mute button with colored dot glow.
  Widget _stemLedButton({
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          height: 24,
          decoration: BoxDecoration(
            color: isActive ? activeColor.withValues(alpha: 0.25) : AppColors.bgInset,
            borderRadius: radius,
            border: Border.all(
              color: isActive ? activeColor.withValues(alpha: 0.7) : AppColors.border,
              width: isActive ? 1.5 : 1,
            ),
            boxShadow: isActive
                ? [BoxShadow(color: activeColor.withValues(alpha: 0.35), blurRadius: 6)]
                : null,
          ),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  shape: BoxShape.circle,
                  color: isActive ? activeColor : AppColors.textMuted.withValues(alpha: 0.3),
                  boxShadow: isActive
                      ? [BoxShadow(color: activeColor.withValues(alpha: 0.8), blurRadius: 3)]
                      : null,
                ),
              ),
...
// ═══════════════════════════════════════════════════════════════════
//  STEM LANE WAVEFORM PAINTER — Mini waveform for each stem track
// ═══════════════════════════════════════════════════════════════════

class _StemLaneWaveformPainter extends CustomPainter {
  final List<double> waveform;
  final Color color;
  final double position;

  _StemLaneWaveformPainter({
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < barCount; i++) {
      ...
      // Upper bar
      canvas.drawRect(
        Rect.fromLTWH(x + 0.3, centerY - barH / 2, barWidth - 0.6, barH / 2),
        paint,
      );

      // Lower bar (mirror)
      canvas.drawRect(
        Rect.fromLTWH(x + 0.3, centerY, barWidth - 0.6, barH / 2),
        paint,
      );
    }
  ...
  @override
  bool shouldRepaint(covariant _StemLaneWaveformPainter oldDelegate) {
