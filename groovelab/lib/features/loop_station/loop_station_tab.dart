...
class _LoopStationTabState extends ConsumerState<LoopStationTab>
  ...
  Widget _buildCompactRecordingTimer() {
    return AnimatedBuilder(animation: _recPulse, builder: (_, __) => Container(
      decoration: BoxDecoration(
        shape: BoxShape.circle,
        color: AppColors.danger.withValues(alpha: 0.03 + _recPulse.value * 0.04),
        // Chrome/metallic outer ring
        border: Border.all(
          color: AppColors.danger.withValues(alpha: 0.35 + _recPulse.value * 0.35),
          width: 3.0,
        ),
        boxShadow: [
          // Outer glow pulsing
          BoxShadow(color: AppColors.danger.withValues(alpha: _recPulse.value * 0.25), blurRadius: 20, spreadRadius: 2),
          // Inner metallic shadow
          BoxShadow(color: const Color(0xFF2A2A2A), blurRadius: 4, offset: const Offset(-2, -2)),
      ...
      child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Container(width: 12, height: 12, decoration: BoxDecoration(shape: BoxShape.circle,
          ...
          boxShadow: [
            BoxShadow(color: AppColors.danger.withValues(alpha: 0.4 + _recPulse.value * 0.4), blurRadius: 8),
          ])),
        const SizedBox(height: 4),
        Text('REC', style: AppFonts.spaceMono(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.danger,
          shadows: [Shadow(color: AppColors.danger.withValues(alpha: 0.5), blurRadius: 6)])),
      ])),
    ));
  }
  ...
  Widget _compactTransportBtn({
    ...
    final isRec = label == 'REC' || label == 'STOP' && isActive;
    return GestureDetector(
      onTap: onTap,
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        AnimatedContainer(
          duration: const Duration(milliseconds: 200),
          width: isRec ? 46 : 42, height: isRec ? 46 : 42,
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            gradient: isActive
                ? RadialGradient(
                    ...
                    radius: 0.8,
                    colors: [ec, Color.lerp(ec, Colors.black, 0.3)!],
                  )
                : RadialGradient(
                    center: const Alignment(0, -0.3),
                    radius: 0.85,
                    colors: [
                      ec.withValues(alpha: enabled ? 0.14 : 0.04),
                      ec.withValues(alpha: enabled ? 0.06 : 0.02),
                    ],
                  ),
            border: Border.all(
              color: isActive
                  ? ec.withValues(alpha: 0.8)
                  : ec.withValues(alpha: enabled ? 0.35 : 0.08),
              width: isActive ? 2.5 : 1.5,
            ),
            boxShadow: [
              if (glow) BoxShadow(color: color.withValues(alpha: 0.55), blurRadius: 18, spreadRadius: 1),
              if (isActive) BoxShadow(color: ec.withValues(alpha: 0.30), blurRadius: 8, spreadRadius: -1),
              // Metallic raised effect
          ...
          child: Icon(icon, size: isRec ? 20 : 18, color: isActive ? AppColors.bgDeepest : ec),
        ),
        const SizedBox(height: 2),
        Text(label, style: AppFonts.outfit(fontSize: 8, fontWeight: FontWeight.w700,
          color: isActive ? ec : ec.withValues(alpha: enabled ? 0.7 : 0.3),
          letterSpacing: 0.5)),
      ]),
    );
  }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          ...
          colors: [Color(0xFF161616), AppColors.bgDark],
        ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: ec.withValues(alpha: 0.08),
          border: Border.all(
            color: ec.withValues(alpha: enabled ? 0.15 : 0.05),
            width: 0.5,
          ),
          boxShadow: enabled
              ? [
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(width: 4),
          Text(label, style: AppFonts.outfit(fontSize: 9, fontWeight: FontWeight.w700, color: ec, letterSpacing: 0.3)),
        ]),
      ),
    );
  }
