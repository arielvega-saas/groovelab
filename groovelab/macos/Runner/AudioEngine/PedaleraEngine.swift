...
/// Professional-quality guitar effects signal chain for macOS using AVAudioEngine.
...
final class PedaleraEngine {
    ...
    private func createNodes(type: String, params: [String: Double], engine: AVAudioEngine, format: AVAudioFormat) -> [AVAudioNode] {
        ...
        case "eq":
            ...
            let g: [Float] = [Float(params["low"] ?? 0), Float(params["lowMid"] ?? 0),
                              Float(params["mid"] ?? 0), Float(params["hiMid"] ?? 0),
                              Float(params["high"] ?? 0)]
            let t: [AVAudioUnitEQFilterType] = [.lowShelf, .parametric, .parametric, .parametric, .highShelf]
            for i in 0..<5 {
                eq.bands[i].filterType = t[i]; eq.bands[i].frequency = f[i]
                eq.bands[i].gain = g[i]; eq.bands[i].bandwidth = 1.0; eq.bands[i].bypass = false
            }
        ...
        case "amp":
            ...
            let gain = params["gain"] ?? 50
            d.loadFactoryPreset(gain > 60 ? .multiDistortedSquared : .multiBrokenSpeaker)
            d.wetDryMix = Float(gain); engine.attach(d)
            let eq = AVAudioUnitEQ(numberOfBands: 3)
            eq.bands[0].filterType = .lowShelf; eq.bands[0].frequency = 200
            eq.bands[0].gain = Float((params["bass"] ?? 50) - 50) / 5
            eq.bands[1].filterType = .parametric; eq.bands[1].frequency = 1000
            eq.bands[1].gain = Float((params["mid"] ?? 50) - 50) / 5; eq.bands[1].bandwidth = 0.8
            eq.bands[2].filterType = .highShelf; eq.bands[2].frequency = 3500
            eq.bands[2].gain = Float((params["treble"] ?? 50) - 50) / 5
        ...
        case "cabinet":
            ...
            // All cabinets share: highpass (remove sub-bass), lowpass (remove fizz)
            applyCabinetCurve(eq: eq, cabinetType: cabinetType)
            let mix = Float(params["mix"] ?? 100) / 100.0
            eq.globalGain = mix * 0.0 // EQ is always 100% wet in chain; mix controls blend
            engine.attach(eq)
        ...
        case "delay":
            ...
            d.delayTime = (params["time"] ?? 400) / 1000.0
            d.feedback = Float(params["feedback"] ?? 35)
            d.wetDryMix = Float(params["mix"] ?? 30)
            d.lowPassCutoff = 15000
            engine.attach(d); return [d]

        // ─── REVERB ─────────────────────────────────────────────────────────
        // Apple built-in reverb with preset mapping + pre-delay
        case "reverb":
            // Pre-delay implemented as a very short delay node
    ...
    private func applyCabinetCurve(eq: AVAudioUnitEQ, cabinetType: Int) {
        // Band 0: High-pass to remove sub-bass rumble
        // Band 1: Low-shelf body
        // Band 2: Low-mid resonance peak
        // Band 3: Mid presence
        // Band 4: High-shelf rolloff (speaker rolloff)
        // Band 5: Low-pass to remove fizz
    ...
    private func startChorusLFO(delay: AVAudioUnitDelay) {
        ...
        timer.setEventHandler { [weak self] in
            ...
            let clampedDelay = max(1.0, min(30.0, delayMs)) / 1000.0 // convert to seconds
    ...
    private func startGateMonitoring(engine: AVAudioEngine, format: AVAudioFormat) {
        ...
        // Install tap on input to monitor signal level
    ...
    func setParam(pedalIndex: Int, paramName: String, value: Double) {
        ...
        case "reverb":
            // Handle pre-delay node if present
    ...
    func setBypass(pedalIndex: Int, bypassed: Bool) {
        ...
        // Use node bypass where possible to avoid audio dropout
        for node in chainNodes[pedalIndex].nodes {
            if let eq = node as? AVAudioUnitEQ {
                ...
                // Gate gain node: set to pass-through when bypassed
                mixer.outputVolume = bypassed ? 1.0 : gateCurrentGain
            } else {
                // Fallback: reconnect chain
                guard let engine = audioEngine, let input = inputNode else { return }
