...
final class MetronomeEngine {
    ...
    private var drumBeats: Int = 4
    private var drumBeatUnit: Int = 4
    private var drumAccentPattern: [Double] = []
    ...
    // Interval training
    private var intervalTrainingEnabled: Bool = false
    private var intervalClickBars: Int = 4
    private var intervalSilentBars: Int = 2
    private var intervalCurrentBar: Int = 0
    private var intervalIsSilent: Bool = false

    // Random silence
    private var randomSilenceEnabled: Bool = false
    private var randomSilenceProbability: Int = 0

    // Volume / mute
    private var metronomeVolume: Float = 1.0
    private var metronomeMuted: Bool = false
    ...
    private func scheduleMetronomeClick(at time: AVAudioTime) {
        ...
        // Track bar boundaries for interval training
        if isMainBeat && mainBeatIndex == 0 && currentSubBeat > 0 {
            intervalCurrentBar += 1
            if intervalTrainingEnabled {
                let threshold = intervalIsSilent ? intervalSilentBars : intervalClickBars
                if intervalCurrentBar >= threshold {
                    intervalIsSilent = !intervalIsSilent
                    intervalCurrentBar = 0
                }
            }
        }

        // Determine if this beat should be silent (interval training or random silence)
        let shouldSilence = (intervalTrainingEnabled && intervalIsSilent) ||
            (randomSilenceEnabled && isMainBeat && Int.random(in: 0..<100) < randomSilenceProbability)

        if isMainBeat {
            let vol = mainBeatIndex < accentPattern.count ? accentPattern[mainBeatIndex] : 0.7
            if vol > 0 && !shouldSilence {
    ...
    func updateDrumTimeSig(beats: Int, beatUnit: Int) {
        audioQueue.async {
            self.drumBeats = beats
            self.drumBeatUnit = beatUnit
        }
    }
    func updateDrumAccentPattern(_ pattern: [Double]) {
        audioQueue.async { self.drumAccentPattern = pattern }
    }
    func updateCountIn(bars: Int) {
        audioQueue.async { self.countInBars = bars }
    }
    func updateIntervalTraining(enabled: Bool, clickBars: Int, silentBars: Int) {
        audioQueue.async {
            self.intervalTrainingEnabled = enabled
            self.intervalClickBars = max(1, clickBars)
            self.intervalSilentBars = max(1, silentBars)
            self.intervalCurrentBar = 0
            self.intervalIsSilent = false
        }
    }
    func updateRandomSilence(enabled: Bool, probability: Int) {
        audioQueue.async {
            self.randomSilenceEnabled = enabled
            self.randomSilenceProbability = max(0, min(100, probability))
        }
    }
    func setMetronomeVolume(_ vol: Double) {
        audioQueue.async {
            self.metronomeVolume = Float(max(0, min(1, vol)))
            self.mixerNode?.outputVolume = self.metronomeMuted ? 0 : self.metronomeVolume
        }
    }
    func setMetronomeMuted(_ muted: Bool) {
        audioQueue.async {
            self.metronomeMuted = muted
            self.mixerNode?.outputVolume = muted ? 0 : self.metronomeVolume
        }
    }
