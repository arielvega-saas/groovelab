...
class _TunerTabState extends ConsumerState<TunerTab> with TickerProviderStateMixin {
  ...
  late Animation<double> _inTuneBurstAnim;
  ...
  void initState() {
    ...
    _inTuneBurstAnim = CurvedAnimation(parent: _inTuneBurstController, curve: Curves.easeOut);
  }
  ...
  void _startListening({String? deviceId}) async {
    ...
    await _audio.startTuner((data) {
      ...
      setState(() {
        ...
        // Trigger in-tune animations on transition
        ...
        builder: (context, constraints) {
          ...
          return Column(
            ...
            children: [
              ...
              // ── Semicircular gauge with radial depth gradient ──
              Stack(
                alignment: Alignment.center,
                children: [
                  // Radial gradient behind gauge for depth
                  Container(
                    width: gaugeWidth + 24,
                    height: gaugeHeight + 16,
                    decoration: BoxDecoration(
                      ...
                      gradient: RadialGradient(
                        ...
                        radius: 1.0,
                        colors: [
                          AppColors.bgInset.withValues(alpha: 0.6),
                          AppColors.bgDeepest.withValues(alpha: 0.0),
                        ],
                      ),
                    ),
                  ),
                  // In-tune radial burst animation
                  if (isInTuneActive)
                    AnimatedBuilder(
                      animation: _inTuneBurstAnim,
                      builder: (_, __) => Container(
                        width: (gaugeWidth * 0.5) + (_inTuneBurstAnim.value * gaugeWidth * 0.4),
                        height: (gaugeHeight * 0.3) + (_inTuneBurstAnim.value * gaugeHeight * 0.5),
                        decoration: BoxDecoration(
                          shape: BoxShape.circle,
                          gradient: RadialGradient(
                            colors: [
                              AppColors.accent2.withValues(alpha: 0.18 * (1.0 - _inTuneBurstAnim.value)),
                  ...
                  Container(
                    ...
                    child: CustomPaint(
                      ...
                      painter: _TunerArcGaugePainter(
                        cents: _smoothCents, noteColor: noteColor, isActive: _frequency > 0, inTune: isInTuneActive,
                      ),
                    ),
                  ),
                ],
              ...
              Row(
                ...
                children: [
                  // Note letter — large LCD style with in-tune pulse
                  AnimatedBuilder(
                    ...
                    child: TweenAnimationBuilder<Color?>(
                      tween: ColorTween(
                        begin: AppColors.textMuted,
                            ...
                            ? (_inTune ? AppColors.accent2
                                : (_cents.abs() <= 10 ? AppColors.warning : (_cents.abs() > 20 ? AppColors.danger : AppColors.warning)))
                            : AppColors.textMuted,
                      ),
                      duration: AppAnimations.medium,
                      curve: AppAnimations.smoothCurve,
                      builder: (context, animColor, _) {
                        final effectiveColor = animColor ?? noteColor;
                        return Container(
                          padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 6),
                          decoration: AppTheme.insetPanel(
                            radius: 12,
                            glowColor: _frequency > 0 ? effectiveColor : null,
                          ),
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
                            crossAxisAlignment: CrossAxisAlignment.baseline,
    ...
    return SizedBox(
      ...
      child: Row(
        ...
        children: List.generate(strings.length, (i) {
          ...
          final isPressed = isSelected && !isDetected;
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  ...
                  width: isTuned ? 1.6 : isHighlighted ? 1.0 : 1.0,
                ),
                boxShadow: isPressed
                    ? AppColors.neumorphicInset(glowColor: accent)
                    : isHighlighted
                        ? [
...
// ═══════════════════════════════════════════════════════════════
//  TUNER ARC GAUGE PAINTER (Professional semicircular gauge)
//  - Color zones: red (outer), yellow (mid), green (center +/-5)
//  - Tick marks at -50, -25, -10, -5, 0, +5, +10, +25, +50
//  - Animated needle with shadow & green-zone glow
// ═══════════════════════════════════════════════════════════════

class _TunerArcGaugePainter extends CustomPainter {
  final double cents;
  final Color noteColor;
  final bool isActive;
  final bool inTune;

  _TunerArcGaugePainter({required this.cents, required this.noteColor, required this.isActive, required this.inTune});

  // Map cent value (-50..+50) to arc fraction (0..1)
  static double _centToFrac(double c) => (c + 50.0) / 100.0;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Zone boundaries in cents: [-50,-25] red, [-25,-5] yellow, [-5,+5] green, [+5,+25] yellow, [+25,+50] red
    const zoneSegments = 60;
    for (int i = 0; i < zoneSegments; i++) {
      final frac = i / zoneSegments;
      final centVal = frac * 100.0 - 50.0; // -50 to +50
      final angle = pi + pi * frac;
      final absCent = centVal.abs();

      Color zoneColor;
      if (absCent <= 5) {
        zoneColor = AppColors.accent2.withValues(alpha: 0.40);
      } else if (absCent <= 25) {
        final t = (absCent - 5) / 20.0; // 0..1 within yellow zone
        zoneColor = Color.lerp(
          AppColors.accent2.withValues(alpha: 0.20),
          AppColors.warning.withValues(alpha: 0.22),
          t,
        )!;
      } else {
        final t = (absCent - 25) / 25.0; // 0..1 within red zone
        zoneColor = Color.lerp(
      ...
      canvas.drawArc(
        Rect.fromCircle(center: center, radius: radius),
        angle, pi / zoneSegments + 0.005, false,
        Paint()..color = zoneColor..style = PaintingStyle.stroke..strokeWidth = 10,
      );
    }

    // ── Tick marks at specific cent positions ──
    const tickCents = [-50.0, -25.0, -10.0, -5.0, 0.0, 5.0, 10.0, 25.0, 50.0];
    const tickLabels = ['-50', '-25', '-10', '-5', '0', '+5', '+10', '+25', '+50'];
    for (int t = 0; t < tickCents.length; t++) {
      final frac = _centToFrac(tickCents[t]);
      final angle = pi + pi * frac;
      final isCenter = tickCents[t] == 0.0;
      final isMajor = tickCents[t].abs() == 50 || tickCents[t].abs() == 25 || isCenter;
      final isMinor5 = tickCents[t].abs() == 5;
      final inner = radius - (isCenter ? 24 : isMajor ? 16 : isMinor5 ? 12 : 10);
      ...
      if (isCenter) {
        ...
        canvas.drawLine(
          ...
          Paint()
            ..color = AppColors.accent2.withValues(alpha: inTune ? 0.55 : 0.30)
            ..strokeWidth = 8
        ...
        Color tickColor;
        if (tickCents[t].abs() <= 5) tickColor = AppColors.accent2.withValues(alpha: 0.50);
        else if (tickCents[t].abs() <= 25) tickColor = AppColors.textMuted;
        else tickColor = AppColors.danger.withValues(alpha: 0.55);
      ...
      if (isMajor || isMinor5) {
        final labelR = radius + (isCenter ? 16 : 15);
        final labelPos = Offset(
          center.dx + labelR * cos(angle),
          center.dy + labelR * sin(angle),
        );
        Color labelColor;
        if (isCenter) labelColor = AppColors.accent2.withValues(alpha: 0.80);
        else if (tickCents[t].abs() <= 5) labelColor = AppColors.accent2.withValues(alpha: 0.50);
        else if (tickCents[t].abs() >= 50) labelColor = AppColors.danger.withValues(alpha: 0.50);
        else labelColor = AppColors.textMuted.withValues(alpha: 0.60);
        _drawTextAt(canvas, tickLabels[t], labelPos, labelColor, isCenter ? 9 : 7);
      }
    }

    // ── Minor tick marks (every 5 cents not already drawn) ──
    for (int c = -50; c <= 50; c += 5) {
      if (tickCents.contains(c.toDouble())) continue;
      final frac = _centToFrac(c.toDouble());
      final angle = pi + pi * frac;
    ...
    // ── Flat/Sharp symbols at edges ──
    _drawTextAt(canvas, '\u266D', Offset(center.dx + (radius + 20) * cos(pi + pi * 0.06), center.dy + (radius + 20) * sin(pi + pi * 0.06)),
      AppColors.danger.withValues(alpha: 0.55), 12);
    _drawTextAt(canvas, '\u266F', Offset(center.dx + (radius + 20) * cos(pi + pi * 0.94), center.dy + (radius + 20) * sin(pi + pi * 0.94)),
      AppColors.danger.withValues(alpha: 0.55), 12);
    ...
    canvas.drawLine(
      ...
      Offset(tipPos.dx + 1.5, tipPos.dy + 1.5),
      Paint()..color = Colors.black.withValues(alpha: 0.40)..strokeWidth = 5..strokeCap = StrokeCap.round,
    );

    // Needle glow (soft halo when in green zone)
    if (inTune) {
      canvas.drawLine(
        center,
        tipPos,
    ...
    // Needle body — tapered from center to tip
    final needlePath = Path();
    final perpAngle = needleAngle + pi / 2;
    const baseHalf = 3.0;
    const tipHalf = 0.8;
    needlePath.moveTo(
      center.dx + baseHalf * cos(perpAngle),
      center.dy + baseHalf * sin(perpAngle),
    );
    needlePath.lineTo(
      tipPos.dx + tipHalf * cos(perpAngle),
      tipPos.dy + tipHalf * sin(perpAngle),
    );
    needlePath.lineTo(
      tipPos.dx - tipHalf * cos(perpAngle),
      tipPos.dy - tipHalf * sin(perpAngle),
    ...
    needlePath.close();
    canvas.drawPath(needlePath, Paint()..color = noteColor);

    // Needle tip glow
    canvas.drawCircle(tipPos, 5, Paint()..color = noteColor.withValues(alpha: 0.35)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 5));
    canvas.drawCircle(tipPos, 2.5, Paint()..color = noteColor);
    canvas.drawCircle(tipPos, 1.2, Paint()..color = Colors.white.withValues(alpha: 0.60));

    // Center pivot — neumorphic raised dot
    canvas.drawCircle(center, 10, Paint()..color = Colors.black.withValues(alpha: 0.60));
    canvas.drawCircle(center, 8, Paint()
      ..shader = const RadialGradient(
        colors: [Color(0xFF2E2E2E), Color(0xFF1A1A1A)],
      ).createShader(Rect.fromCircle(center: center, radius: 8)));
    // LED glow on pivot when in tune
    if (inTune) {
      canvas.drawCircle(center, 10, Paint()
        ..color = AppColors.accent2.withValues(alpha: 0.30)
        ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 8));
    }
    canvas.drawCircle(center, 5, Paint()..color = noteColor);
    canvas.drawCircle(center, 2.5, Paint()..color = Colors.white.withValues(alpha: 0.55));
  }
  ...
  @override
  bool shouldRepaint(covariant _TunerArcGaugePainter old) =>
    old.cents != cents || old.noteColor != noteColor || old.isActive != isActive || old.inTune != inTune;
}
