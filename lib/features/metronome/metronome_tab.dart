...
class _MetronomeTabState extends ConsumerState<MetronomeTab> {
                                ...
                                child: GestureDetector(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text('TAP TO EDIT', style: AppFonts.outfit(
                                        fontSize: 7,
                                        color: playing
  ...
  Widget _buildLcdInfoBar(String clickSound, int currentBeat, String tempoName) {
    final lcdLabel = AppFonts.robotoMono(
      fontSize: 7, fontWeight: FontWeight.w500,
      color: const Color(0xFF3A4E2A), letterSpacing: 1.2,
    );
    final lcdValue = AppFonts.robotoMono(
      fontSize: 10, fontWeight: FontWeight.w700,
      color: const Color(0xFF8EC828),
      shadows: [
        Shadow(color: const Color(0xFF8EC828).withValues(alpha: 0.50), blurRadius: 6),
        Shadow(color: const Color(0xFF8EC828).withValues(alpha: 0.20), blurRadius: 14),
      ],
    );
    ...
    return Container(
      ...
      child: Stack(
        children: [
          ...
          Positioned.fill(
            child: CustomPaint(painter: _LcdScanlinePainter()),
          ),
          // ── LCD content ──
          Padding(
...
class _KnobPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // ── Outer tick marks ring — small lines every 30 degrees ──
    const outerTickCount = 12;
    final outerTickR = outerR + 2;
    for (int t = 0; t < outerTickCount; t++) {
      final angle = (t / outerTickCount) * 2 * pi;
      final innerTickR = outerTickR - 6;
      canvas.drawLine(
        Offset(center.dx + innerTickR * cos(angle), center.dy + innerTickR * sin(angle)),
        Offset(center.dx + outerTickR * cos(angle), center.dy + outerTickR * sin(angle)),
        Paint()
    ...
    // ── Tempo zone color arc — behind the knob ring ──
    const startAngle = 2.35619; // 135 degrees
    const sweepAngle = 4.71239; // 270 degrees
    final arcR = outerR + 0.5;
    final arcRect = Rect.fromCircle(center: center, radius: arcR);

    // Slow zone (blue/cyan) 0-40%
    canvas.drawArc(arcRect, startAngle, sweepAngle * 0.4, false, Paint()
      ..color = const Color(0xFF0077CC).withValues(alpha: 0.25)
      ..style = PaintingStyle.stroke
      ..strokeWidth = 3.0
      ..strokeCap = StrokeCap.butt);
    // Medium zone (cyan) 40-60%
    canvas.drawArc(arcRect, startAngle + sweepAngle * 0.4, sweepAngle * 0.2, false, Paint()
      ..color = AppColors.accent.withValues(alpha: 0.30)
      ..style = PaintingStyle.stroke
      ..strokeWidth = 3.0
      ..strokeCap = StrokeCap.butt);
    // Fast zone (orange) 60-85%
    canvas.drawArc(arcRect, startAngle + sweepAngle * 0.6, sweepAngle * 0.25, false, Paint()
      ..color = AppColors.warm.withValues(alpha: 0.30)
      ..style = PaintingStyle.stroke
      ..strokeWidth = 3.0
      ..strokeCap = StrokeCap.butt);
    // Very fast zone (red) 85-100%
    canvas.drawArc(arcRect, startAngle + sweepAngle * 0.85, sweepAngle * 0.15, false, Paint()
    ...
    // ── Enhanced inner neumorphic shadow ring — deeper 3D effect ──
    // Outer dark shadow edge
    canvas.drawCircle(center, innerR + 2, Paint()
    ...
    canvas.drawCircle(center, innerR, Paint()
      ..color = const Color(0xFF0A0A0A).withValues(alpha: 0.60)
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1.5);
    // Inner light bevel (reflected ambient)
    canvas.drawCircle(center, innerR - 1, Paint()
      ..color = const Color(0xFF2A2A2A).withValues(alpha: 0.50)
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1.5);
    // Subtle inner glow ring
    canvas.drawCircle(center, innerR - 2, Paint()
      ..color = const Color(0xFF333333).withValues(alpha: 0.20)
      ..style = PaintingStyle.stroke
      ..strokeWidth = 0.8);
...
class _BeatBlocksDisplay extends StatefulWidget {
  final int numBeats;
  ...
  @override
  State<_BeatBlocksDisplay> createState() => _BeatBlocksDisplayState();
}

class _BeatBlocksDisplayState extends State<_BeatBlocksDisplay>
  ...
  @override
  void didUpdateWidget(covariant _BeatBlocksDisplay oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.currentBeat != widget.currentBeat && widget.isPlaying) {
      _previousBeat = oldWidget.currentBeat;
      _triggerBounce(widget.currentBeat);
    }
  ...
  void _triggerBounce(int beatIndex) {
    if (!_scaleControllers.containsKey(beatIndex)) {
      final controller = AnimationController(
        vsync: this,
        duration: AppAnimations.medium,
      );
      _scaleControllers[beatIndex] = controller;
      _scaleAnimations[beatIndex] = TweenSequence<double>([
        TweenSequenceItem(tween: Tween(begin: 1.0, end: 1.15), weight: 35),
        TweenSequenceItem(tween: Tween(begin: 1.15, end: 1.0)
            .chain(CurveTween(curve: AppAnimations.springCurve)), weight: 65),
      ]).animate(controller);
    }
    _scaleControllers[beatIndex]!.forward(from: 0.0);
  }
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: List.generate(widget.numBeats, (i) {
            final isAccent = i < widget.accents.length && widget.accents[i] >= 0.9;
            final isMuted  = i < widget.accents.length && widget.accents[i] < 0.3;
            final isActive = widget.isPlaying && widget.currentBeat == i;
            final isPreviousBeat = widget.isPlaying && _previousBeat == i;
            ...
            // Afterglow color for previous beat
            final afterglowColor = isPreviousBeat && !isActive
                ? barColor.withValues(alpha: 0.25)
                : restColor;

            // Active bar height (full), rest bar height (fraction of container)
            // Reserve: padding (6+6=12) + dot spacing (3) + dot max (5) = 20
            final maxH   = widget.height - 20;
            final activeH = isAccent ? maxH : maxH * 0.82;
            final restH   = isMuted  ? 4.0  : maxH * 0.28;
            // Previous beat gets a taller rest for afterglow trail effect
            final afterglowH = isPreviousBeat && !isActive
                ? maxH * 0.50
                : restH;

            Widget block = Expanded(
              child: GestureDetector(
                onTap: () => widget.onTapBeat(i),
                child: Padding(
                    ...
                    children: [
                      ...
                      AnimatedContainer(
                        duration: isActive
                            ...
                            : isPreviousBeat
                                ? const Duration(milliseconds: 450) // slower afterglow decay
                                : const Duration(milliseconds: 380), // slow phosphor decay
                        curve: Curves.easeOut,
                        width: double.infinity,
                        height: isActive ? activeH : afterglowH,
                        decoration: BoxDecoration(
                          borderRadius: BorderRadius.circular(3),
                          color: isActive ? barColor : afterglowColor,
                          boxShadow: isActive
                              ? [
                                  BoxShadow(
                                  ...
                                  BoxShadow(
                                    color: barColor.withValues(alpha: 0.40),
                                    blurRadius: 28,
                                    spreadRadius: 5,
                                  ),
                                ]
                      ...
                      AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          ...
                          color: isActive
                              ? barColor
                              : isPreviousBeat
                                  ? barColor.withValues(alpha: 0.40)
                                  : isMuted
            ...
            // Wrap active beat with scale bounce animation
            if (_scaleAnimations.containsKey(i)) {
...
/// LCD scanline overlay painter — draws subtle horizontal lines for retro LCD effect.
class _LcdScanlinePainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
    ...
    final overlayPaint = Paint()
      ..shader = LinearGradient(
        ...
        colors: [
          const Color(0xFF8EC828).withValues(alpha: 0.03),
          Colors.transparent,
          const Color(0xFF8EC828).withValues(alpha: 0.02),
        ],
