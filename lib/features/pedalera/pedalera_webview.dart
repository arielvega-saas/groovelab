...
class _PedaleraWebViewState extends ConsumerState<PedaleraWebView>
  ...
  Future<void> _startLocalServer() async {
    _localhostServer = InAppLocalhostServer(port: 8765);
    try {
      await _localhostServer!.start();
      if (mounted) setState(() {});
    } catch (e) {
      debugPrint('PedaleraWebView: failed to start local server: $e');
      if (mounted) {
        setState(() {
          _hasError = true;
  ...
  Widget _buildWebPlatform() {
    // On web, use HtmlElementView with an iframe pointing to the asset.
    const viewType = 'pedalera-webview';
    // ignore: undefined_prefixed_name
    // The registration must happen once; typically done via platformViewRegistry
    // in main.dart or with a conditional import. Here we assume it is already
    // registered with:
    //   ui.platformViewRegistry.registerViewFactory(
    //     'pedalera-webview',
    //     (int viewId) => html.IFrameElement()
    //       ..src = 'assets/$_assetPath'
    //       ..style.border = 'none'
    //       ..style.width = '100%'
    //       ..style.height = '100%',
    //   );
  ...
  Widget _buildNativePlatform() {
    ...
    const url = 'http://localhost:8765/assets/pedalera/pedalboard.html';
